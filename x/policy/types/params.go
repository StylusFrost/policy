...
const (
	...
	// DefaultMaxRegoCodeSize limit max bytes read to prevent gzip bombs
	DefaultMaxRegoCodeSize = 600 * 1024
)

var ParamStoreKeyUploadAccess = []byte("uploadAccess")
var ParamStoreKeyInstantiateAccess = []byte("instantiateAccess")
var ParamStoreKeyMaxRegoCodeSize = []byte("maxRegoCodeSize")
...
// DefaultParams returns default policy parameters
func DefaultParams() Params {
	return Params{
		RegoUploadAccess:             AllowEverybody,
		InstantiateDefaultPermission: AccessTypeEverybody,
		MaxRegoCodeSize:              DefaultMaxRegoCodeSize,
	}
}
...
func (p *Params) ParamSetPairs() paramtypes.ParamSetPairs {
	return paramtypes.ParamSetPairs{
		paramtypes.NewParamSetPair(ParamStoreKeyUploadAccess, &p.RegoUploadAccess, validateAccessConfig),
		paramtypes.NewParamSetPair(ParamStoreKeyInstantiateAccess, &p.InstantiateDefaultPermission, validateAccessType),
		paramtypes.NewParamSetPair(ParamStoreKeyMaxRegoCodeSize, &p.MaxRegoCodeSize, validateMaxRegoCodeSize),
	}
}

// ValidateBasic performs basic validation on policy parameters
func (p Params) ValidateBasic() error {
	...
	if err := validateAccessConfig(p.RegoUploadAccess); err != nil {
		return errors.Wrap(err, "upload access")
	}
	if err := validateMaxRegoCodeSize(p.MaxRegoCodeSize); err != nil {
		return errors.Wrap(err, "max rego code size")
	}
	return nil
}
...
func validateMaxRegoCodeSize(i interface{}) error {
	a, ok := i.(uint64)
	if !ok {
